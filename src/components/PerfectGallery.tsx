...
function useFavorites() {
  const [favs, setFavs] = useState<Set<string>>(() => {
    ...
    if (hash.startsWith('#perfect-favs=')) {
      const indices = decodeFavs(hash.slice(14));
      if (indices.length > 0) {
        const shared = new Set(indices.filter(i => i < perfectPhotos.length).map(i => perfectPhotos[i].src));
        localStorage.setItem('perfect-wedding-favorites', JSON.stringify([...shared]));
        return shared;
      }
    }
    try {
      const stored = localStorage.getItem('perfect-wedding-favorites');
  ...
  const toggle = useCallback((src: string) => {
    setFavs(prev => {
      ...
      localStorage.setItem('perfect-wedding-favorites', JSON.stringify([...next]));
      return next;
    });
  }, []);

  const shareUrl = useCallback(() => {
    const indices = perfectPhotos.map((p, i) => favs.has(p.src) ? i : -1).filter(i => i >= 0);
    if (indices.length === 0) return null;
    const base = window.location.origin + window.location.pathname;
    return `${base}#perfect-favs=${encodeFavs(indices)}`;
  }, [favs]);
  ...
  return (
    <div ref={ref} className={`photo-card ${visible ? 'visible' : ''}`}>
      <div className="photo-wrapper" onClick={onClick}>
        <img src={src} alt="Perfect Wedding" loading="lazy" />
      </div>
      <button
...
export default function PerfectGallery() {
  const { favs, toggle, shareUrl } = useFavorites();
  const [filter, setFilter] = useState<string>(() => {
    return window.location.hash.startsWith('#perfect-favs=') ? 'favorites' : 'all';
  });
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);
  const [copied, setCopied] = useState(false);

  const filtered = filter === 'favorites'
    ? perfectPhotos.filter(p => favs.has(p.src))
    : perfectPhotos;
